...
_BULK_CHUNK_ROWS = 20000
_SELECT_CHUNK_KEYS = 500


def _bulk_insert_or_match(
    session: Session,
    model: type[ModelT],
    primary_key: str,
    rows: Sequence[dict],
    ...
    """Set-based `_insert_or_match` over one homogeneous batch of row dicts.

    Existing rows must match the candidate on every compared field, exactly as
    the row-at-a-time path enforces; the first conflicting row (in input
    order) raises with the same message.
    """
    ...
    existing_by_key: dict[object, Any] = {}
    keys = [row[primary_key] for row in rows]
    for start in range(0, len(keys), _SELECT_CHUNK_KEYS):
        chunk = keys[start : start + _SELECT_CHUNK_KEYS]
        for existing in session.execute(select(table).where(key_column.in_(chunk))):
            existing_by_key[getattr(existing, primary_key)] = existing
    inserted = 0
    reused = 0
    pending: list[dict] = []
    pending_by_key: dict[object, dict] = {}
    for row in rows:
        key = row[primary_key]
        existing = existing_by_key.get(key)
        if existing is None and key in pending_by_key:
            existing = pending_by_key[key]
        if existing is not None:
            lookup = existing.get if isinstance(existing, dict) else (
                lambda field, source=existing: getattr(source, field)
            )
            conflicts = [
                field
                for field in fields
                if _normalise_compare(lookup(field)) != _normalise_compare(row[field])
            ]
            if conflicts:
                raise PointInTimeIngestionError(
        ...
        if len(pending) >= _BULK_CHUNK_ROWS:
...
def persist_bundle(
    ...
    *,
    ...
    with session_scope(session_factory) as session:
        ...
        inserted, reused = _bulk_insert_or_match(
            session,
            SecurityIdentifier,
            "identifier_id",
            identifier_rows,
            ...
            delisting_rows.append(
                {
                    ...
                    "successor_security_id": (
                        successor.security_id if successor else None
                    ),
